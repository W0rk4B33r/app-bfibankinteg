	...
	return ("com.apptech.app-bankinteg.controller.AppUI5",{
		...
		*/
		...
		generateDocNum: function (sTableName) {
			...
			$.ajax({
				url: "https://xsjs.biotechfarms.net/app-xsjs/ExecQuery.xsjs?dbName="+ jQuery.sap.storage.Storage.get("dataBase") +"&procName=SPAPP_RE_GETDOCNUM&tableName=" +
					sTableName,
				type: "GET",
				dataType: "json",
				async: false,
		...
		getAllByColumn: function (tableName, columnName, columnValue) {
			var getAllByColumnURL = "https://xsjs.biotechfarms.net/app-xsjs/ExecQuery.xsjs?dbName=APP_RE&procName=SPAPP_RE_GETALLDATA_BYCOL&tableName=" +
		...
		generateNumber: function (sDocType) {
			...
			switch (sDocType) {
			case "Quote":
				generateNumberURL = "https://xsjs.biotechfarms.net/app-xsjs/ExecQuery.xsjs?dbName=APP_RE&procName=SPAPP_RE_GENERATENUMBER&DocType=Quote";
				break;
			case "Reservation":
				generateNumberURL = "https://xsjs.biotechfarms.net/app-xsjs/ExecQuery.xsjs?dbName=APP_RE&procName=SPAPP_RE_GENERATENUMBER&DocType=Reservation";
				break;
			case "Contract":
				generateNumberURL = "https://xsjs.biotechfarms.net/app-xsjs/ExecQuery.xsjs?dbName=APP_RE&procName=SPAPP_RE_GENERATENUMBER&DocType=Contract";
				break;
			}
			var returnCode = [];
			$.ajax({
		...
		getAllDataByKeyAJAX: function (sTableName, sKeyCode, sQueryType) {
			var aReturnResult = [];
			$.ajax({
				url: "https://xsjs.biotechfarms.net/app-xsjs/ExecQuery.xsjs?dbName=APP_RE&procName=SPAPP_RE_GETALLDATA_BYKEY&tableName=" + sTableName +
		...
		getHANAData: function (sModule, sQueryType, sKey, sKey2) {
			var aReturnResult = [];
			var sURL = "https://xsjs.biotechfarms.net/app-xsjs/ExecQuery.xsjs?dbName=APP_RE&procName=SPAPP_RE_" + sModule + "&queryType=" + sQueryType +
		...
		getAllDataByColAJAX: function (sTableName, sColName, sColValue, sQueryType) {
			var aReturnResult = [];
			$.ajax({
				url: "https://xsjs.biotechfarms.net/app-xsjs/ExecQuery.xsjs?dbName=APP_RE&procName=SPAPP_RE_GETALLDATA_BYCOL&tableName=" + sTableName +
